...
class CRM_Core_Form extends HTML_QuickForm_Page {
  ...
  private static function selectOrAnyPlaceholder($props, $required) {
    ...
    return ts('- select %1 -', $tsPlaceholder);
  }
  ...
  public function addEntityRef($name, $label = '', $props = [], $required = FALSE) {
    ...
    $props['placeholder'] = $props['placeholder'] ?? ts('- select %1 -', [1 => ts(str_replace('_', ' ', $props['entity']))]);
  ...
  public function addChainSelect($elementName, $settings = []) {
    $label = CRM_Core_DAO_AllCoreTables::entityLabel(strpos($elementName, 'rovince') ? 'StateProvince' : 'County');
    $props = $settings += [
